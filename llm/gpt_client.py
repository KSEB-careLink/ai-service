...
def generate_quiz_only(patient_name, photo_description: str, relation: str, tone: ToneEnum, image_path: str, category: str):
    allowed_terms = extract_terms(photo_description)
    if not allowed_terms:
        allowed_terms = [relation]
    ...
    category_guidelines = {
        "가족": """
- 가족 구성원, 가족 행사, 가족과의 추억에 중점
- 예: "누구와 함께 있었나요?", "어떤 가족 행사였나요?", "가족 중 누가 ~했나요?"
- 가족 관계 호칭을 활용한 문제 (아버지, 어머니, 형제, 자매 등)
        """,
        "동네": """
- 지역 이름, 동네 장소, 근처 상점이나 시설에 중점  
- 예: "어느 동네에서 ~했나요?", "근처에 있던 가게는?", "그 지역의 유명한 곳은?"
- 구체적인 장소명이나 지역 특징을 활용한 문제
        """,
        "학창시절": """
- 학교 이름, 학년, 친구 이름, 선생님, 학교 행사에 중점
- 예: "어느 학교에서~?", "몇 학년 때~?", "어떤 과목 시간이었나요?", "누구 선생님이셨나요?"
- 교육과 관련된 활동이나 학교 시설 활용
        """,
        "여행": """
- 여행지, 교통수단, 숙소, 여행 동반자에 중점
- 예: "어디로 여행을 갔었나요?", "어떤 교통편으로~?", "어디서 머물렀나요?"
- 지역 명소나 여행 관련 활동 중심
        """,
        "환자가 좋아하는 것": """
- 취미, 좋아하는 음식, 관심사, 특기에 중점
- 예: "어떤 취미를 즐겼나요?", "좋아하는 음식은?", "자주 하던 활동은?"
- 개인적 선호나 특별한 관심사 활용
        """
    }

    current_category_guide = category_guidelines.get(category, category_guidelines["가족"])
...
## ❓ 퀴즈 작성 규칙
- **카테고리 "{category}"에 맞는 문제 유형을 우선적으로 사용하세요.**
- 위의 카테고리 가이드라인을 참고하여 해당 분야에 특화된 질문을 만드세요.
- 다음 유형 중 카테고리와 어울리는 것을 선택해 구성하세요:
  1. 이름 맞추기 (장소, 사람, 물건)
  2. 시각 회상 (상황 묘사)  
  3. 자유 회상 (기억 속 요소 선택)
  4. 카테고리별 특화 질문 (위 가이드라인 참조)

- 퀴즈는 보호자가 자연스럽게 회상하듯 묻는 말투로 작성하세요.
- 퀴즈도 tone에 맞는 말투를 유지하되, 절대 창작 감정 넣지 마세요.
- **3개 문제는 모두 다른 유형으로 다양하게 구성하세요.**
...
🧠 마지막으로, 위 회상 문장을 바탕으로 **"{category}" 카테고리에 특화된 퀴즈를 총 3개** 만들어줘.  
**각 문제는 서로 다른 유형과 관점으로 구성**하여 다양성을 확보하세요.

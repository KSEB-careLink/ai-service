...
def run_one(csv, outdir, window, hidden, layers, batch, epochs, lr, seed=42, bidir=False):
    ...
    ckpt_new = run_dir / "model_daily_10_and_month.pth"
    ckpt_old = run_dir / "model_daily_nextday.pth"
    ckpt_path = ckpt_new if ckpt_new.exists() else ckpt_old

    if not metrics_path.exists() or not ckpt_path.exists():
        print(f"[WARN] missing outputs: metrics={metrics_path.exists()} ckpt={ckpt_path.exists()}")
        return None

    with open(metrics_path, "r", encoding="utf-8") as f:
        hist = json.load(f)
    val_mse_seq = hist.get("val_mse_total") or hist.get("val_mse") or []
    val_mae10_seq = hist.get("val_mae10") or hist.get("val_mae") or []
    val_mae_month_seq = hist.get("val_mae_month") or []

    best_val_mse_total = float(min(val_mse_seq)) if val_mse_seq else float("inf")
    best_val_mae10 = float(min(val_mae10_seq)) if val_mae10_seq else float("inf")
    best_val_mae_month = float(min(val_mae_month_seq)) if val_mae_month_seq else float("inf")
    ...
    return {
        ...
        "lr": lr,
        "bidir": bidir,
        "best_val_mse_total": best_val_mse_total,
        "best_val_mae10": best_val_mae10,
        "best_val_mae_month": best_val_mae_month,
        "ckpt": str(ckpt_path)
    }
...
def main():
    ...
    ap.add_argument("--trials", type=int, default=0, help="0이면 전체 그리드, >0이면 랜덤 샘플링 횟수")
    ap.add_argument("--seed", type=int, default=42)
    ap.add_argument("--mode", choices=["slide_exact", "grid"], default="grid",
                    help="slide_exact: 슬라이드 표의 2개 구성만 / grid: 전체 탐색")
    args = ap.parse_args()
    ...
    if args.mode == "slide_exact":
        ...
        grid = list(itertools.product(windows, hiddens, layers_list, batches, epochs_list, bidir_list, lrs))
        candidates = random.sample(grid, k=min(args.trials, len(grid))) if args.trials > 0 else grid
    ...
    if results:
        def key_fn(r):
            return (r["best_val_mse_total"], r["best_val_mae10"], r["best_val_mae_month"])
        best = min(results, key=key_fn)
        print("\n[BEST CONFIG]")
        for k in ["run_id","window","hidden","layers","batch","epochs","lr","bidir",
                  "best_val_mse_total","best_val_mae10","best_val_mae_month","path","ckpt"]:
            print(f"- {k}: {best[k]}")
    else:

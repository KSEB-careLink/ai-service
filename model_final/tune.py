...
def run_one(csv, outdir, window, hidden, layers, batch, epochs, seed=42, bidir=False):
    run_id = f"w{window}_h{hidden}_L{layers}_b{batch}_e{epochs}_s{seed}_{uuid.uuid4().hex[:8]}"
...
def main():
    ...
    grid = list(itertools.product(windows, hiddens, layers_list, batches, epochs_list, bidir_list))
    ...
    results = []
    for (W, H, L, B, E, BD) in candidates:
        try:
            res = run_one(args.csv, args.outdir, W, H, L, B, E, seed=args.seed, bidir=BD)
    ...
    if results:
        ...
        print("\n[BEST CONFIG]")
        for k in ["run_id","window","hidden","layers","batch","epochs","bidir","best_val_mse","best_val_mae","path"]:
            print(f"- {k}: {best[k]}")
    else:

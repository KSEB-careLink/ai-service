...
CKPT_PATH = os.getenv("MODEL_CKPT", os.path.join(MODEL_DIR, "model.pth")) # 이름 경로 수정 필요하면 하기 
...
_model = LSTMRegMulti(input_dim=F, hidden=_hidden)
...
def load_logs_from_api(patient_id: str) -> pd.DataFrame:
    try:
        ...
        if response.status_code == 404:
            ...
            raise HTTPException(status_code=response.status_code, detail=f"Node.js API 오류: {response.text}")
        ...
        if isinstance(data, dict) and "data" in data:
            ...
            raise HTTPException(status_code=500, detail="예상하지 못한 API 응답 형식")
        ...
        if missing_columns:
            raise HTTPException(status_code=500, detail=f"API 응답에서 누락된 컬럼: {missing_columns}")
...
def predict_accuracy_live(
    ...
    try:
        ...
        # 2) created_at UTC 표준화
        if not df.empty:
            df["created_at"] = ensure_utc_series(df["created_at"])
            df = df.dropna(subset=["created_at"]).sort_values("created_at").reset_index(drop=True)

        # 3) KST 월 경계 → UTC 윈도우 계산
        start_utc, end_utc = kst_month_window_utc(month)
        wnd_end = start_utc - timedelta(seconds=1)
        wnd_start = wnd_end - timedelta(days=W-1)

        # 4) 데이터 없는 경우 → Cold start
        if df.empty:
            X = _build_features(pd.DataFrame(columns=["date","daily_acc_rate","daily_avg_time"]),
                                wnd_start, wnd_end, W)
            with torch.no_grad():
                x = torch.tensor(X, dtype=torch.float32).unsqueeze(0)
                y10, y_month = _model(x)
                raw_pred10 = y10.squeeze(0).tolist()
                raw_pred_month = float(y_month.item())
            blended = BASELINE_ACC
            return {
                "patient_id": patient_id,
                "month": month,
                "predicted_final_accuracy": round(blended,4),
                "predicted_next10_days": [round(v,4) for v in raw_pred10],
                "cold_start": True,
        ...
        with torch.no_grad():
            ...
            raw_pred10 = y10.squeeze(0).tolist()
            raw_pred_month = float(y_month.item())
        raw_pred_month = max(0.0, min(1.0, raw_pred_month))

        # 8) 커버리지 계산
        attempts = df[(df["created_at"] >= wnd_start) & (df["created_at"] <= wnd_end)].shape[0]
        active_days = daily[(daily["date"] >= wnd_start) & (daily["date"] <= wnd_end)].shape[0]
        ...
        # 9) Cold start 판정 + blending
        cold_start_flag = not (active_days >= COLD_DAYS_OK or attempts >= COLD_ATTEMPTS_OK)
        blended_month = coverage * raw_pred_month + (1.0 - coverage) * BASELINE_ACC

        return {
            "patient_id": patient_id,
            "month": month,
            "predicted_final_accuracy": round(blended_month,4),
            "predicted_next10_days": [round(v,4) for v in raw_pred10],
